import { defineComponent, useModel, mergeProps, mergeModels, useSSRContext, computed, unref, withCtx, createTextVNode, toDisplayString, ref, isRef } from "vue";
import { ssrRenderAttrs, ssrRenderClass, ssrIncludeBooleanAttr, ssrInterpolate, ssrRenderList, ssrRenderComponent } from "vue/server-renderer";
...
const _sfc_main$2 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ])}"${ssrIncludeBooleanAttr(__props.disabled) ? " disabled" : ""}>${ssrInterpolate(__props.monthlyLabel ?? "Monthly")}</button><button type="button" class="${ssrRenderClass([
        model.value ? "bg-white dark:bg-gray-700 text-primary-dark dark:text-accent-teal shadow" : "text-gray-600 dark:text-gray-400",
        "rounded-full px-4 py-2 text-sm font-medium transition-all"
      ])}"${ssrIncludeBooleanAttr(__props.disabled) ? " disabled" : ""}>${ssrInterpolate(__props.annualLabel ?? "Annual")}</button>`);
      if (__props.disabled) {
...
//# sourceMappingURL=pricing-DWG4Nt-5.js.map
